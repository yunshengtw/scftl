...
fuse_getattr(const char *path, struct stat *stbuf)
{
  ...
  if (strcmp(path, stats) == 0) {
    sprintf(statsbuf, "nread %ld\nnwrite %ld\nnflush %ld\nngetattr %ld\nnopen %ld\nnrelease %ld\nnreadir %ld\nnfuse_read %ld\nnfuse_write %ld\nncreate %ld\nntrunc %ld\nnmkdir %ld\nnrmdir %ld\nnunlink %ld\nnfsync %ld\nnfsyncdir %ld\n", fsstats.nread, 
	    ...
	    fsstats.nmkdir,  fsstats.nrmdir, fsstats.nunlink, fsstats.nfuse_fsync, fsstats.nfuse_fsyncdir);
...
void ideinit(void)
{
  flash_init();
  disk_recovery();
  printf("[xv6fs-xlog] disk recovered\n");
}

void
unix_read(int sector, unsigned char *buf, int sz)
{
  disk_read(sector, (uint32_t *)buf);
  fsstats.nread++;
}
...
{
  disk_write(sector, (uint32_t *)buf);
  fsstats.nwrite++;
}
...
main(int argc, char **argv)
{
  ...
  */
  ...
  fprintf(stderr, "The xv6 file system is ready.\n");
  /*
  int i;
  for (i = 2; i < argc; i++) {
    argv[i-1] = argv[i];
